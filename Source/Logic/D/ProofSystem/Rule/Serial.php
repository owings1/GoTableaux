 ...
 * @package Logics
 */
class Serial extends \GoTableaux\ProofSystem\TableauxSystem\Rule\Branch
{
	public function appliesToBranch( Branch $branch, Logic $logic )
	{
		foreach ( $branch->getIndexes() as $i )
			if ( !$branch->find( 'exists', array( 'i' => $i, 'j' => '*' ))) return true;
		return false;
	}
	...
	{
		foreach ( $branch->getIndexes() as $i )
			if ( !$branch->find( 'exists', array( 'i' => $i, 'j' => '*' ))) {
				$branch->createNode( 'Access', array( 'i' => $i, 'j' => $branch->newIndex() ));
				return;
			}
	}
