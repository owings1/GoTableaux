...
class NegatedPossibility extends \GoTableaux\ProofSystem\TableauxSystem\Rule\Branch
{
	...
	{
		$potentialNodes = $branch->find( 'all', array( 'sentenceForm' => '~PA', 'i' => '*', 'ticked' => false ));
		foreach ( $potentialNodes as $node ) {
			$sentence = $this->getSentence( $node, $logic );
	...
	{
		$branch->createNode( 'Modal Sentence', array( 'sentence' => $logic->parseSentence( '~PA' ), 'i' => 0 ))
			   ->createNode( 'Access', array( 'i' => 0, 'j' => 1 ));
	}
	
	private function getSentence( Node $node, Logic $logic )
	{
		list( $operand ) = $node->getSentence()->getOperands();
		list( $operand ) = $operand->getOperands();
		return $logic->negate( $operand );
	}
}