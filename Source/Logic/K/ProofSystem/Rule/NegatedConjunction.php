 ...
 * @package CPL
 */
class NegatedConjunction extends \GoTableaux\ProofSystem\TableauxSystem\Rule\Node
{
	...
	public function applyToNode( Node $node, Branch $branch, Logic $logic )
	{
		...
		$branch->branch()
			   ->createNode( 'Modal Sentence', array( 'sentence' => $logic->negate( $leftConjunct ), 'i' => $i ))
			   ->tickNode( $node );
			
		$branch->createNode( 'Modal Sentence', array( 'sentence' => $logic->negate( $rightConjunct ), 'i' => $i ))
			   ->tickNode( $node );
	}
}